		...
		foreach ($weapons as $defindex => $default) { ?>
			...
			if(array_key_exists($defindex, $selectedSkins)){
				getrarity($skins[$defindex][$selectedSkins[$defindex]['weapon_paint_id']]["paint_name"]); }?>">
						...
						?>
						<div class="card-footer">
						<form class="skin-update" action="" method="POST" id="<?php print_r($skins[$defindex][0]['weapon_name'])?>">
						<div id="<?php print_r($skins[$defindex][0]['weapon_name'])?>" class="custom-select" style="width:200px;">
					...
					<div class="modal" id="weaponModal<?php echo $defindex ?>" tabindex="-1" role="dialog" aria-labelledby="weaponModalLabel<?php echo $defindex ?>" aria-hidden="true">
